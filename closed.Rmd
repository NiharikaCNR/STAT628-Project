...
### Read data
```{r ,message=FALSE}
close_data<-read_csv("~/Desktop/closed/closed_business_reviews.csv")
close_bad_review<-read_csv("~/Desktop/closed/close-bad-review.csv")
close_good_review <- read_csv("~/Desktop/closed/close-good-review.csv")
...
## close bad review analysis

```{r,message=FALSE,warning=FALSE}
tidy_bad_data<-close_bad_review%>%
  #select(-user_id,-review_id)%>%
  unnest_tokens(word,text)%>% 
  anti_join(stop_words)%>%
  mutate(word = str_remove_all(word, "\\d+"))%>%
